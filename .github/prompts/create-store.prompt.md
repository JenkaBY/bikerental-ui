...
2. Create mapper functions in the `src/app/core/mappers` directory; If object contains reference (eg. field name could contain `slug`, `type` in name) on lookup entity,
   inject proper a lookup store and map object itself instead of having reference.
