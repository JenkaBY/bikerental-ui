# Task 012: Agreement List Component (Smart, Route Page)

> **Applied Skills:** `angular-signals` (manual signal-based sort instead of `MatSort` — no
> existing admin list uses `MatSort`, so a `computed()` over a `sortState` signal keeps the signal
> idiom, per FR-01's design "prefer manual sort on signals to stay in the signal idiom"),
> `angular-di` (`providers: [AgreementTemplateStore]` — feature-scoped, opens dialogs with
> `viewContainerRef` so the store is visible to them), `error-handling` (`ApiErrorParser.parse` +
> `resolveErrorMessage` + `NotificationService`; ALWAYS `store.load()` after a 409 on
> activate/delete so the stale row disappears) — implements the route page per FR-01's design
> section 3 (`agreement-list.component.ts` bullet).

## 1. Objective

Create `AgreementListComponent`: `MatTable` over `store.templates()`, sortable on `versionNumber`
and `createdAt`, status chip badges, header "New template" button, row actions gated by status
(DRAFT → edit/activate/delete; ACTIVE/DEACTIVATED → view read-only), activate/delete via
`ConfirmDialogComponent`, full error handling with mandatory reload on 409.

## 2. File to Modify / Create

* **File Path:** `projects/admin/src/app/agreements/agreement-list.component.ts`
...
**Note on `providers: [AgreementTemplateStore]`:** this is a **feature** provider on the component,
not `providedIn: 'root'` on the store itself (contrast `TariffStore`) — every navigation to this
route gets a fresh store instance, and dialogs opened with `viewContainerRef: this.viewContainerRef`
resolve the same instance via Angular Material's dialog injector resolution.

**Note on manual sort:** no existing admin list in this codebase uses `MatSort`
(`tariff-list.component.ts`, `users-list.component.ts` do not import it) — this component follows
suit with a `computed()` sort over `sortColumn`/`sortDirection` signals, keeping the signal idiom
per FR-01's design guidance rather than introducing the first `MatSort` usage in admin.

**Note on 409 handling:** both `activate()` and `delete()` call `this.load()` unconditionally on
`apiError.status === 409` (covers `not_activatable`, `not_deletable`, and
`concurrent_activation`), per FR-01's design ("ALWAYS `load()` on 409 so the stale row
disappears").
